...
class JetQGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, input_path:str, train:bool=False)->torch.utils.data.Dataset:
        """_summary_
        ...
        print(f"Files: {len(glob(self.input_path))}")
        self.len = 0
        i = 0
        for path in glob(self.input_path):
            if i==5:break
            print(np.load(path)['X'].shape)
            self.len += np.load(path)['X'].shape[0]
            i += 1

        files = glob(self.input_path)[:i]

        self.data = [np.load(inp_path)['X']  for inp_path in files]
        self.label = [np.load(inp_path)['y'] for inp_path in files]
    ...
    # it's easier for the network to predict the log of the observable, shifted and scaled
        ...
        obs = np.log10((event[:,0]*(event[:,1:3]**2).sum(1)/event[:,0].sum()).sum())
        pt, eta, phi, mass = event[:,0], event[:,1], event[:,2], np.zeros_like(event[:,0])
        ...
        PID = event[:,3]
        part_pid = PID
        part_isCHPlus = ak.values_astype((PID == 211) + (PID == 321) + (PID == 2212), 'float32')
        part_isCHMinus = ak.values_astype((PID == -211) + (PID == -321) + (PID == -2212), 'float32')
        part_isNeutralHadron = ak.values_astype((PID == 130) + (PID == 2112) + (PID == -2112), 'float32')
        ...
        part_charge = (part_isCHPlus + part_isEPlus + part_isMuPlus) -(part_isCHMinus + part_isEMinus + part_isMuMinus)

        #6
        cat_features = [part_isNeutralHadron.to_numpy(),
        ...
        #15
        list_features = [pt, eta, phi,
                         energy.to_numpy(),deta.to_numpy(), dphi.to_numpy(),
                         px.to_numpy(), py.to_numpy(), pz.to_numpy(),
                         part_log_pt, part_e_log,
                         
                         np.log(part_pt/jet_pt), np.log(part_e/jet_energy), np.sqrt(deta.to_numpy()**2+ dphi.to_numpy()**2),
                        
                         part_pid,
                         ]

        #7
        jet_features = [jet_eta, jet_phi, jet_pt, jet_mass, jet_energy, jet_nparticles, obs]
        res = torch.stack([torch.Tensor(i)  for i in list_features])
        jet_features = torch.stack([torch.full([jet_nparticles], i) for i in jet_features])
        ...
        event = Data(x=res.to(dtype=torch.float).view( res.shape),
                    pos=pos.to(dtype=torch.float).view( pos.shape))
        #Compute the Edge features:
        knn_graph = torch_geometric.transforms.knn_graph.KNNGraph(k=8, num_workers=12)#4)
        ...
        dR = torch.sqrt((res[1][edge_index[0]])**2 + (res[2][edge_index[1]])**2)
        k_T =  torch.min(res[0][edge_index[0]], res[0][edge_index[1]])*dR
        z = torch.min(res[0][edge_index[0]], res[0][edge_index[1]])/(res[0][edge_index[0]] + res[0][edge_index[1]])
        m_2 = -torch.abs(res[0][edge_index[0]]- res[0][edge_index[1]])**2 + (res[3][edge_index[0]] + res[3][edge_index[1]])
