...
class CombineHiddens(Module):
    def __init__(self, dim, num_hiddens_to_concat):
        super().__init__()
...
class HRM(Module):
    def __init__(
        ...
        *,
        dim,
        num_tokens,
        reasoning_steps=5,  # N in the paper - the number of forward evals for the last network (highest hierarchy) above
        relative_period: (
            int | tuple[int, ...]
        ) = 2,  # the relative period for each network evaluation call to the one just previous - in the paper, they do 2 networks with a period of 2
        causal=True,
        ignore_index=-1,
    ):
        super().__init__()
        ...
        self.hidden_combiners = ModuleList(
            [
                CombineHiddens(dim, self.num_networks + 1 - network_index)
                for network_index in range(self.num_networks)
            ]
        )
    ...
    def forward(
        ...
        *,
        ...
        tokens = seq  # mixer_x = self.encoder(mixer_x)
        # self.to_input_embed(seq)

        # handle hiddens

        if not exists(hiddens):
            hiddens = torch.zeros_like(seq)
            hiddens = repeat(
                hiddens, "... -> num_networks ...", num_networks=self.num_networks
            )
        ...
        for index in range(total_low_steps):
            ...
            context = (
                torch.no_grad if one_step_grad and not is_last_step else nullcontext
            )
            ...
            with context():
                ...
                for network_index, (
                    ...
                    zip(self.networks, self.hidden_combiners, self.evaluate_networks_at)
                ):

                    if not divisible_by(iteration, evaluate_network_at):
                        continue

                    all_hiddens = (tokens, *hiddens_dict.values())
        ...
        loss = F.cross_entropy(
            rearrange(pred, "b n l -> b l n"), labels, ignore_index=self.ignore_index
        )
